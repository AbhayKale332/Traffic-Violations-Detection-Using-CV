...
import { Upload, X, Smartphone, AlertCircle, Loader2 } from "lucide-react";

const ROBOFLOW_API_KEY = "8uvtxZId3oOxVg80LO8f";
const MODEL_ID = "seat_belt-and-mobile-vbve5/1";
...
export default function SeatbeltMobileDetection() {
  const [imageSrc, setImageSrc] = useState<string | null>(null);
  const [fileName, setFileName] = useState<string>("");
  const [result, setResult] = useState<InferenceResult | null>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [tintedDetected, setTintedDetected] = useState(false);
  const fileRef = useRef<HTMLInputElement>(null);
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const imgRef = useRef<HTMLImageElement>(null);

  /** Analyse average brightness of the image using an offscreen canvas.
   *  Returns a value 0-255. Dark tinted windows typically fall below ~60. */
  function analyseImageBrightness(): number {
    const img = imgRef.current;
    if (!img) return 255;
    const offCanvas = document.createElement("canvas");
    // Sample at a smaller size for speed
    offCanvas.width = 80;
    offCanvas.height = 80;
  ...
  async function runInference() {
    ...
    try {
      ...
      // If the model found nothing, check brightness for tinted windows
      if (data.predictions.length === 0) {
        const brightness = analyseImageBrightness();
        // Dark image (< 65 avg luminance) with no detections → likely tinted
        setTintedDetected(brightness < 65);
      }
    } catch (err) {
  ...
  const mobileCount =
    result?.predictions.filter((p) =>
      p.class.toLowerCase().includes("mobile")
    ).length ?? 0;
  
  const windsheildCount =
    result?.predictions.filter((p) =>
  ...
  const isTintedWindow = tintedDetected || (windsheildCount > 0 && !hasPersonOrDetails);
          ...
          <div className="w-full max-w-lg space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="rounded-lg border border-red-100 bg-red-50 px-4 py-3 flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs text-red-700 font-medium">Mobile Use</p>
              ...
              <div className="rounded-lg border border-blue-100 bg-blue-50 px-4 py-3 flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs text-blue-700 font-medium">Windshield</p>
